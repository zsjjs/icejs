...
export function wrapperPageWithSSR(context, routes, appConfig) {
  ...
  const { app: { parseQueryParams } } = appConfig;
  const WrapperPageFn = () => {
    const ServerWrapperedPage = (props) => {
      const queryParams = getQueryParams(parseQueryParams, props.location.search);
      const MatchedPageComponent = getComponentByPath(routes, context.pathname);
      return <MatchedPageComponent {...Object.assign({}, props, pageInitialProps, { queryParams })} />;
    };
...
export function wrapperPageWithCSR(appConfig) {
  const wrapperPage = (PageComponent) => {
    ...
    const RouterWrapperedPage = (props) => {
      ...
      return <PageComponent {...Object.assign({}, props, data, { queryParams })} />;
    };
...
function getQueryParams(parseQueryParams, locationSearch) {
  let queryParams = {};
  if (parseQueryParams) {
    queryParams = queryString.parse(locationSearch);
  }
  return queryParams;
}
